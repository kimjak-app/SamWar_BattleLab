...
- Moved only low-risk Economy / City pure helper logic selected from the v0.71 function map; city state mutation, turn income mutation, storage/resource mutation, save/load, BattleContext, pending invasion, turn orchestration, selection state mutation, and scene-node-heavy UI functions remain in `scripts/worldmap_test.gd`.
- Existing scene files, script paths, main scene, gameplay formulas, save/load schema, BattleContext schema, pending invasion schema, enemy research, AI research, ship/siege persistent storage, assets, and imports were not changed.
- Next task: `v0.71-07 Defense / Battle Helper Extraction`.
...
- Confirmed `scenes/WorldMap_Test.tscn` is absent in this repo, so the existing stable root scene path was used without moving or renaming scene files.
- This is an entry-point switch only; no scene file rename, scene node change, script refactor, folder split, gameplay behavior change, save/load schema change, BattleContext schema change, pending invasion schema change, battle formula change, diplomacy/spy formula change, enemy research, AI research, ship/siege persistent storage, asset, or import behavior changed.
- Test-name cleanup is intentionally deferred to `v0.71-02 Test Name Cleanup`.
- Next task: `v0.71-02 Test Name Cleanup`.
...
- Labor loop decision: city-only accumulated resource with cap, future key `labor_pool`, generated per selected PLAYER city from city capacity factors.
- Policy loop decision: national-only accumulated resource with cap, future key `policy_points`, generated per PLAYER nation from administrative and political factors.
- Both resources remain design-only in v0.70-90: no persistent key, no save/load schema change, no generation implementation, no actual charge implementation, and no UI behavior change.
- Current actual charge remains `gold + food group`; city food group order remains `rice -> barley -> seafood`.
- Future roadmap: save schema draft, generation MVP, UI display MVP, actual charge MVP, then F6 QA record.
- No runtime code, gameplay logic, UI behavior, save/load schema, active research payload, cost/duration/effect balance values, actual charge logic, deduction order, paid cost state, cancel/refund, BattleContext, pending invasion schema, scene, asset, or import behavior was changed.
- Next candidates: `v0.70-91 Labor Policy Save Schema Draft`, `v0.70-91 City Research Category Identity Polish`, `v0.70-91 Domestic Tech Edge Case Polish`, or `v0.70-91 Labor Policy Generation Formula Draft`.

## v0.70-89 Labor Policy Resource State Design
- Baseline: `v0.70-88 Domestic Tech Balance F6 Result Record` at `c782e3a14b0bc2e6c2d1046e7a8a118b4e5adf1d`; tracked files were clean at task start.
- Added `agent/DOMESTIC_TECH_LABOR_POLICY_RESOURCE_DESIGN.md` to lock the Domestic Tech `labor` / `policy` resource-state decision before implementation.
- Conclusion: MVP keeps `labor` and `policy` as display-only planned costs. They are not actual charge targets in v0.70-89.
- Future preferred direction is city-only `labor_pool` for labor and national-only `policy_points` for policy, after separate save/load schema, turn recovery/production, UI policy, old-save compatibility, and manual QA design.
- Current actual charge remains `gold + food group`; city food group order remains `rice -> barley -> seafood`; national tech keeps the confirmed gold-charge-centered model.
- No runtime code, gameplay logic, UI behavior, save/load schema, active research payload, cost/duration/effect balance values, actual charge logic, deduction order, paid cost state, cancel/refund, BattleContext, pending invasion schema, scene, asset, or import behavior was changed.
- Next candidates: `v0.70-90 Labor Policy Cost UI Label Polish`, `v0.70-90 City Research Category Identity Polish`, `v0.70-90 Domestic Tech Edge Case Polish`, or `v0.70-90 Labor Policy Save Schema Draft`.
...
- Available and affordable research now displays `필요 비용: ... · 시작 시 차감` instead of any display-only style wording.
- Available but insufficient resources now surface `자원이 부족해 연구를 시작할 수 없습니다.` with `부족: 금 N / 군량 N` shortage details, and the action button uses `자원 부족`.
- Researching, completed, and locked/prerequisite states keep status priority over cost wording; action button labels are shortened to `연구 중`, `완료`, or `조건 부족`.
- Added side-effect-free UI wording QA flags to `_get_domestic_tech_research_actual_charge_summary_mvp()`.
- No actual charge logic, deduction amount/order, research start flow, active payload schema, paid cost state, cancel/refund, per-turn charge, completion charge, BattleContext, pending invasion schema, assets/imports, tech definitions, or scene layout was changed.
- Next candidates: `v0.70-85 Research Cost Affordability F6 UI QA Record`, `v0.70-85 Labor Policy Resource State Design`, or `v0.70-85 Domestic Tech Actual Charge Edge Case QA Polish`.

## v0.70-83 Domestic Tech Actual Charge F6 Result Record
- Baseline: `v0.70-82 Domestic Tech Actual Charge Manual QA` at `34baf66a48caf8725556ee618c681e3b58f60355`; tracked files were clean at task start.
- Recorded Kimjak's v0.70-82 Domestic Tech actual charge manual QA result as PASS.
- Confirmed PASS items: National sufficient gold, National insufficient gold, National no per-turn charge, National no completion charge, City sufficient gold/food, City insufficient gold, City insufficient food, City food deduction order, Other city unchanged, Existing active no retroactive charge, Active payload schema unchanged, Enemy/unknown no research/effect, and Godot Output clean.
- This is a result-record pass only. No gameplay code, UI, scene, asset/import, save/load schema, active payload schema, paid cost state, cancel/refund, enemy research/effect, BattleContext, pending invasion schema, or tech definition change was made.
- Next candidates: `v0.70-84 Research Cost Affordability UI Polish`, `v0.70-84 Labor Policy Resource State Design`, or `v0.70-84 Domestic Tech Actual Charge Edge Case QA Polish`.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Domestic Tech research cost display Safe Set:
  - Added `_format_domestic_tech_research_cost_display_mvp()` as the common display formatter for planned gold/food/labor/policy costs.
  - Kept national and city cost display separated through `_get_domestic_tech_research_cost_plan_mvp()`: national tier costs are gold-only and city tier costs are gold/food.
  - Standardized formatter output order as gold / food / labor / policy with Korean UI labels `금`, `군량`, `노역`, and `정책`; zero values are hidden and zero-cost plans remain marked `표시 전용`.
  - Tightened Domestic Tech inspector state rules: completed hides cost, researching prioritizes remaining/total turns, available shows duration plus expected cost, and locked states do not show cost shortage or cost-gating copy.
  - Added `_get_domestic_tech_research_cost_display_summary_mvp()` and aligned research balance flags with `cost_display_only = true`, no charge, no blocking, no paid state, and no affordability check.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Domestic Tech cost/research balance planning:
  - Switched Domestic Tech duration hints and new research duration fallback to tier-based planning: Tier 1 = 2 turns, Tier 2 = 3, Tier 3 = 4, Tier 4 = 5, Tier 5 = 6.
  - Preserved existing active research compatibility by not shrinking already stored `duration_turns`; `remaining_turns` still clamps within the stored duration.
  - Added `_get_domestic_tech_research_cost_plan_mvp()` as display-only planned cost data for national/city techs, with `cost_charged`, `cost_charged_on_start`, `cost_charged_per_turn`, and `cost_blocks_research_start` all false.
  - Updated Domestic Tech inspector copy to show `연구 소요` and `예상 비용 ... (표시 전용)` without adding resource payment or cost gating.
  - Added `_get_domestic_tech_research_balance_summary_mvp()` for QA confirmation of tier duration rules, display-only cost planning, unchanged active/completion flow, and enemy research disabled.
- Preserved scope: no actual research cost application, no cost-based research blocking, no paid-cost state, no battle/diplomacy/spy/market/city_intel/AI formula mutation, no enemy research/effect, no troop/ship/siege count mutation, no BattleContext/pending invasion change, no tech id/name/category/branch/prerequisite change, and no asset/import change.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Domestic Tech full integration QA:
  - Added `_get_domestic_tech_full_effect_integration_summary_mvp()` to report the combined Economy, Military/Defense, National Policy, Naval/Siege, and Diplomacy/Spy Safe Set status in one QA dictionary.
  - Kept completed-only, researching no-effect, PLAYER only, same-city only, display-safe-only, bonus non-persistence, and source uniqueness flags consolidated.
  - Normalized source-tech output one more time for economy, military/defense, and national policy helpers before return.
  - Polished UI section labels to `테크 경제`, `테크 군사/방어`, `테크 국가정책`, `테크 해군/공성`, and `테크 외교/첩보`, with zero/empty sections still hidden and source names compacted.
- Preserved scope: no battle/diplomacy/spy/market/city_intel/AI formula mutation, no troop/ship/siege count mutation, no BattleContext/pending invasion change, no enemy effect/display, no tech definition change, and no asset/import change.
- Existing locks remain: economy Safe Set, military/defense Safe Set, national policy Safe Set, naval/siege Display Safe Set, diplomacy/spy Display Safe Set, UI64 icon priority, node-click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, active/completed normalize, and research start/progress/completion.
...
- Baseline: `v0.70-72 Domestic Tech Diplomacy/Spy Display Safe Set` at `23b90f4cf0f27bd7d9123a9504f96f05ebd7a2b3`; local HEAD matched `origin/main` and tracked files were clean at task start.
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Diplomacy/Spy Display QA polish:
  - Kept `_get_domestic_tech_diplomacy_spy_bonus_mvp()` PLAYER completed national tech only, side-effect free, source-unique, and non-persistent.
  - Strengthened `_get_domestic_tech_city_spy_intel_bonus_mvp(city_id)` and city display formatting so the current empty city spy/intel Safe Set returns no local effects and no false city-detail section.
  - QA summary now reports city spy/intel source uniqueness, empty city spy/intel mapping status, and `empty_city_spy_intel_mapping_no_display` alongside display counters.
  - Left national panel, Domestic Tech inspector, and selected PLAYER city detail remain aligned with the same helpers and avoid success-rate/relation/city_intel wording.
- Preserved scope: no actual diplomacy success modifier, no actual spy success modifier, no relation mutation, no city_intel visibility change, no spy detection or spy action result change, no AI/enemy effect, no BattleContext/pending invasion change, no market/trade formula change, no tech definition change, and no asset/import change.
...
- Next candidate: `v0.70-73 Domestic Tech Full Effect Integration QA & Balance Pass`.
...
- Naval/Siege Display Safe Set:
  - Added PLAYER-city-only `_get_domestic_tech_city_naval_siege_bonus_mvp(city_id)` derived from completed city techs for the same city; no computed naval/siege bonus state is saved.
  - Added display Safe Set mapping for current city tech ids: `commerce_shipyard`, naval training/warship/panokseon/turtle/crane/fire/cannon techs, and `mil_siege_unit` / `mil_siege_engine`.
  - Added city detail and Domestic Tech inspector display lines for selected PLAYER city naval/siege preparation bonuses and unique source tech names.
  - Added effect-status copy for naval/siege Safe Set techs without implying actual battle formula connection.
  - QA summary now reports `naval_siege_effects_enabled`, `naval_display_effects_applied`, `siege_display_effects_applied`, `ship_count_effects_applied = 0`, and `siege_weapon_count_effects_applied = 0`.
- Preserved scope: no actual naval/siege battle modifier, no battle formula change, no troop stat/count mutation, no ship/siege weapon count mutation, no BattleContext change, no pending invasion change, no diplomacy/spy/market/trade effect, no enemy city effect/display, no AI research, no tech definition change, and no asset/import change.
- Existing locks remain: economy Safe Set, military/defense Safe Set, national policy Safe Set, UI64 icon priority, node-click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, active/completed normalize, and research start/progress/completion.
- Next candidates: `v0.70-71-hotfix1 Naval/Siege Display Effect QA Polish` or `v0.70-72 Domestic Tech Diplomacy/Spy Display Safe Set`.
- Manual F6 QA remains required for city naval/siege bonus, same-city only, enemy no-effect, no battle mutation, no ship/siege count mutation, UI bonus display, economy Safe Set, military/defense Safe Set, national policy Safe Set, research flow, icon visibility, click latency, overlay lifecycle, and warning cleanliness.
...
- National Policy Safe Set:
  - Added a side-effect-free PLAYER completed national tech helper that derives policy bonuses from `_player_state["national_domestic_tech_completed"]` each time and never persists computed bonus state.
  - Added Safe Set mapping for law, bureaucracy, centralization, tax reform, conscription, logistics, population policy, foundation storage, and national monopoly.
  - Connected `tax_gold_percent` minimally to PLAYER city gold income through the existing Domestic Tech economy bonus path; final gold remains clamped to `0+`.
  - Kept admin, recruit, logistics, population, law/order, and storage values as UI/QA display or preparation effects only.
  - Left national panel and Domestic Tech inspector now display active national policy bonus lines and unique source national tech names.
  - QA summary now reports national policy counters plus `battle_effects_applied = 0`, `diplomacy_effects_applied = 0`, `spy_effects_applied = 0`, `market_effects_applied = 0`, and `enemy_effects_applied = 0`.
- Preserved scope: no enemy national effect, no AI research, no diplomacy/spy/market formula change, no battle formula change, no troop stat/count mutation, no BattleContext change, no pending invasion change, no tech definition change, and no asset/import change.
- Existing locks remain: economy Safe Set, military/defense Safe Set, UI64 icon priority, node-click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, active/completed normalize, and research start/progress/completion.
- Next candidates: `v0.70-70-hotfix1 National Policy Effect QA Polish` or `v0.70-71 Domestic Tech Diplomacy/Spy Display Safe Set`.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Military/Defense Safe Set scope:
  - Added PLAYER-city-only Domestic Tech military/defense bonus calculation from completed city techs; no bonus state is saved.
  - Added display-safe training/recruitment values for barracks, infantry, archer, and cavalry training techs.
  - Added display-safe city defense values for wall, moat, watchtower, beacon, iron gate, and fortress techs.
  - City detail and Domestic Tech inspector now display selected PLAYER city military/defense bonus lines and source tech names.
  - QA summary now reports `city_defense_effects_applied`, `training_display_effects_applied`, and `battle_effects_applied = 0`.
- Preserved scope: no battle formula change, no troop stat mutation, no troop count auto increase, no BattleContext change, no pending invasion change, no enemy city effect/display, no naval/siege numeric effect, no diplomacy/spy/market/trade effect, no tech definition change, and no asset/import change.
- Existing locks remain: economy Safe Set, UI64 icon priority, node-click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, active/completed normalize, and research start/progress/completion.
- Next candidates: `v0.70-69-hotfix1 Military/Defense Effect QA Polish` or `v0.70-70 Domestic Tech National Policy Effects Safe Set`.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Phase 1 actual effect scope:
  - Completed national tech remains the only source for city `required_national_techs` condition satisfaction.
  - Completed city tech remains scoped to the same city for city prerequisites; other cities do not receive prerequisite credit.
  - Added an explicit required-national completion helper to keep active/researching national techs from unlocking city techs.
  - Inspector relation lines now show completed-based unlock/enhance states such as 해금됨, 해금 가능, 조건 충족, and 연구 필요.
  - `effect_stub` stays display-only and now shows effect description plus 적용 준비/후속 버전 수치 반영 안내.
  - Added an internal Phase 1 effect summary helper with `numeric_effects_applied = 0`.
- Preserved scope: no resource/income/troop/battle/diplomacy/spy/market formula change, no actual numeric effect, no resource payment, no AI research, no enemy research/effect, no BattleContext change, and no pending invasion change.
- Existing locks remain: UI64 icon priority, hotfix4 node-click latency behavior, overlay lifecycle, left PLAYER national tree, right selected PLAYER city tree, enemy/insufficient-intel hiding, active/completed normalize, and research start/progress/completion.
- Next candidates: `v0.70-67-hotfix1 Domestic Tech Effect Phase 1 QA Polish` or `v0.70-68 Domestic Tech Numeric Effects Phase 1 - Economy Safe Set`.
- Manual F6 QA remains required for national-to-city unlock, same-city prerequisite only, relation display, effect display, resource/troop/battle/diplomacy/spy/market invariance, enemy-city safety, icon visibility, click latency, overlay lifecycle, and warning cleanliness.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Runtime scope:
  - Added conservative `ENEMY_FACTION_STRATEGIC_GOAL_SEEDS` for current non-player factions with goal id, compact label, target cities, region hints, pressure, and bounded `1.00..1.15` weight.
  - Unknown factions and PLAYER fall back to default/empty goal behavior; PLAYER does not receive strategic goal scoring.
  - Missing target city ids are ignored by helper guards.
  - Goal seed influence is limited to small reinforcement target bonuses, strategic follow-up type/target scoring bonuses, eligible invasion pair scoring bonuses, and compact summary/hint display metadata.
- Preserved scope: v0.70-51 enemy turn chain, v0.70-52 personality seed scope, v0.70-53 tuning guard, no full enemy AI, no enemy spy actual damage, no enemy diplomacy alliance/trade simulation, no enemy economy simulation, no `ENEMY_INVASION_CHANCE` or `ENEMY_INVASION_MIN_ATTACKER_CITY_TROOPS` change, no pending invasion payload or BattleContext shape change, no replay guard change, no market/alliance/wedge/player action change, no `city_intel` mutation, no scene/asset/`.uid`/`.ogv` changes.
- Verification: `git diff --check`, required guard keyword search, warning-cleanup regression search, project headless load, `WorldMap_Test.tscn` headless load, and `Battle_Fullscreen_Test.tscn` headless load passed. Battle scene emitted existing debug output only.
- Manual F6 QA remains required:
  1. 턴 종료 후 enemy reinforcement가 기존처럼 정상 적용되는지 확인.
  2. strategic goal label이 summary/log에서 너무 길지 않은지 확인.
  3. 고구려/몽골/오다 등 군사·공격 목표 faction이 target/frontline/invasion을 조금 더 선호하는지 확인.
  4. 신라/촉/도쿠가와 등 외교·방어 목표 faction이 diplomacy/defensive behavior를 조금 더 선호하는지 확인.
  5. 규슈 등 첩보 목표 faction이 spy pressure를 조금 더 선호하는지 확인.
  ...
  11. save/load 후 goal-based action/scoring이 replay되지 않는지 확인.
...
- Baseline: `v0.70-52 Enemy Faction Personality Seed MVP` at `4ee00833ac7ea4f953ec6e006362ff51b361551f`.
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- QA / tuning scope:
  - Audited `ENEMY_FACTION_PERSONALITY_SEEDS`, fallback helpers, reinforcement target scoring, strategic action selection, invasion pair scoring, summary/hint display, and replay/save-load guard boundaries.
  - Added explicit `chu` default-balanced seed so every current `FACTION_LABELS` non-player id has a direct profile or a documented fallback.
  - Converted `kyushu_faction` from generic defensive to compact `계략` / `schemer_pressure` so the personality set has a visible spy-pressure leaning profile without adding new spy systems.
  - Tuned spy-pressure strategic selection down slightly so default spy candidates do not drown out diplomatic profiles just from troop/frontline bonuses.
  - Added an invasion scoring guard so personality multipliers only affect positive eligible-pair scores; negative/poor invasion pairs no longer get accidentally improved by sub-1.0 defensive/diplomatic weights.
...
- Manual F6 QA remains required:
  1. 턴 종료 후 enemy reinforcement가 기존처럼 정상 적용되는지 확인.
  2. personality label이 summary/log에 너무 길지 않은지 확인.
  3. 군사/공격 성향의 frontline/invasion 선호가 너무 과하지 않은지 확인.
  4. 외교 성향의 diplomacy follow-up 선호가 너무 과하지 않은지 확인.
  5. 첩보 성향의 spy pressure 선호가 너무 과하지 않은지 확인.
  6. 방어/무역 성향이 침공을 완전히 안 하게 되지 않는지 확인.
  7. strategic action은 여전히 한 턴 최대 1건인지 확인.
  8. enemy spy pressure가 player stat/resource/city_intel을 직접 변경하지 않는지 확인.
  9. enemy diplomacy가 PLAYER relation을 직접 변경하지 않는지 확인.
  10. pending invasion 중 strategic action이 skip되는지 확인.
  11. v0.70-49 invasion guard와 v0.70-51 replay guard가 유지되는지 확인.
  12. save/load 후 personality-based action이 replay되지 않는지 확인.
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- City Detail `외교·첩보 > 첩보` now includes executable `이간질` using action id `wedge`.
- Wedge validation reuses the existing spy target/chancellor/political aptitude/cooldown/iron-wall gates and adds automatic non-player counterpart selection.
- Counterpart selection excludes PLAYER and the target faction, then prioritizes allied target-counterpart relations, score 60+ relations, active alliance/trade-agreement metadata, and finally the highest remaining relation score.
- Wedge cost is `gold 600` and `silk 150`; validation failures do not spend resources, while rolled attempts spend the operation cost and apply spy cooldown.
- Success lowers the target-counterpart relation score by a conservative political-aptitude-scaled delta; if an allied pair drops below `ALLIANCE_ACCEPTANCE_THRESHOLD`, the alliance duration is cleared and the relation returns to neutral.
- Detection uses the existing player-target relation penalty path with `SPY_DETECTED_RELATION_PENALTY_WEDGE`; success and detection can occur together.
- Results are stored in `_player_state["last_spy_wedge_result"]` and shown in recent spy summaries; save/load restores display/state only and never replays costs, relation changes, detection penalties, or alliance break effects.
- Preserved existing `정탐`, `민심 교란`, `성 충성도 교란`, and `반란 조장` behavior; v0.70-40 alliance proposal flow; v0.70-39 market pricing; enemy intel visibility; player chancellor candidate scope; `_player_state["faction_chancellors"]`; left panel scope; right selected-city scope; BattleContext; Selected City Panel; `project.godot`; scenes; assets; `.uid`; and `.ogv`.
- The `v0.70-34-hotfix1` shadowing warning cleanup remains intact.
- Manual F6 QA remains required for wedge button/status, non-player counterpart selection, insufficient-cost failure, success relation decrease, alliance break threshold, detection penalty, spy cooldown, save/load display/state, existing spy/diplomacy/trade regressions, enemy intel lock, chancellor candidate scope, left panel scope, and warning cleanliness.
- Next candidate work:
...
- Removed the local `selected_city_id` shadowing warning in `scripts/worldmap_test.gd` by renaming the diplomacy/spy local to `current_selected_city_id`.
- Removed the built-in-name collision warning by renaming the internal transfer formatter parameter from `sign` to `transfer_multiplier`.
- Removed the local `loyalty_card` shadowing warning in `scripts/worldmap_city_info_panel.gd` by renaming the layout-order local to `selected_loyalty_anchor`.
- Functionality, UI layout, formulas, save/load structure, trade persistence, manual trade, internal transfer, external execution, BattleContext, `project.godot`, scenes, and assets were not changed.
- Next candidate work:
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- City Detail `무역 > 타국무역` now shows only foreign neighboring cities as external trade candidates when the selected city is player-owned.
- Player-owned neighboring cities are excluded from the external trade candidate list and remain the responsibility of `자국무역`.
- External trade display is centered on trade candidates, relation status, trade availability, and relation-based trade efficiency.
- Relation status UI is localized: `allied` -> `동맹`, `neutral` -> `중립`, `hostile` -> `적대`, and `suspended` -> `교역 중단`.
- If there are no adjacent foreign trade candidates, the tab shows an empty state with no forced recent-trade/result details.
- Public support, city loyalty, loyalty drift, seasonal loyalty, revolt risk, troop movement, recruitment/conscription, military supply judgment, and supply-adjustment details are not displayed on the external trade tab.
- Trade leadership remains a future information slot for `재상 위임 / 수동 조정`; no automatic or manual external-trade behavior was implemented.
- Resource tab, city storage cards, internal trade tab, diplomacy/spy tab, Selected City Panel, battle/BattleContext, save/load schema, formulas, `project.godot`, and assets were not changed.
- Next candidate work:
...
- Modified files: `scripts/worldmap_test.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- City Detail `자원` tab now separates resource potential from city-held storage: the existing star display remains `자원 잠재력`, and a new `성 창고` summary shows current city storage.
- Added city runtime `storage` normalization helpers. The structure uses resource ids such as `gold`, `rice`, `barley`, `seafood`, `wood`, `iron`, `horses`, `silk`, and `salt`.
- Hanseong's default city storage is initialized from the current national `resource_stock`: rice 300, barley 250, seafood 80, wood 100, iron 50, horses 30, silk 30, salt 50, gold 500.
- Other cities default to zero storage unless a runtime/loaded city payload provides explicit `storage`, avoiding new balancing or production assumptions.
- City runtime save/load now includes `storage`; older saves without `storage` safely receive default city storage on load.
- Resource tab `성 창고` summary groups food, strategy, and specialty totals and uses simple MVP states: 300+ `안정`, 100-299 `주의`, below 100 `부족`.
- National warehouse UI, national `resource_stock`, trade, turn production, supply consumption, recruitment, battle, BattleContext, city formulas, `project.godot`, scenes, and assets were not changed.
- Manual F6 QA remains required for Hanseong resource tab storage display, save/load persistence, expanded/collapsed drag, tab switching, help, recruitment/governor/attack buttons, and clean Godot Output.
- Next candidate work:
...
- Modified files: `WorldMap_Test.tscn`, `scripts/worldmap_test.gd`, `scripts/worldmap_city_info_panel.gd`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, and `agent/WORLDMAP_RULES.md`.
- Governor assignment: added `GovernorAssignOption` inside the selected-city `GovernorCard`; candidates are limited to the selected city's `stationed_hero_ids`, with `미임명` as the first option.
- Assignment connection: `WorldMapCityInfoPanel` emits `governor_assignment_requested(city_id, governor_id)`, and `scripts/worldmap_test.gd` updates the mutable city runtime `governor_id` without moving heroes, changing `stationed_hero_ids`, or enforcing global duplicate-governor rules.
- Governor policy connection: existing `GovernorPolicyOption` remains the policy selector and continues to update `_city_policy_state[city_id]`; developer copy was replaced with player-facing policy names/descriptions.
- Save/load: city runtime save now includes `governor_id` and `governor_policy_id`; top-level `city_policy_state` is serialized/loaded with fallback to seed city data for older saves.
- Removed selected-city governor policy copy that exposed `재상 정책 수행`, `Godot에서는 표시 전용`, placeholder/no-effect wording, or "No city stat or turn effect applied" logs.
- Excluded scope: no governor exclusivity, hero movement, wounded/captured/dead governor release, domestic/trade/relation formula changes, turn-income/security redesign, city ownership/troop/resource formula changes, battle scene changes, or `project.godot` changes.
- Verification result: `git diff --check`, Godot headless project load, `WorldMap_Test.tscn` headless load, `Battle_Fullscreen_Test.tscn` headless load, and docs marker check passed before commit. Manual F6 QA remains recommended for dropdown interaction and save/load behavior.
- Next candidate work:
  1. `v0.70-20 WorldMap Selected City Panel Troop Stats Polish`
  2. `v0.70-21 WorldMap City Detail Panel Right Side Polish`
  3. `v0.70-22 WorldMap Battle Entry Camera Zoom Handoff`
