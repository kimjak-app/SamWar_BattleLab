class_name WorldMapGarrisonCompactController
extends Node
...
func _ensure_view() -> void:
	if _compact_scroll != null and is_instance_valid(_compact_scroll):
		return
	var card := world_map.get_node_or_null(GARRISON_CARD_PATH) as PanelContainer
	if card == null:
		return
...
func _refresh_if_needed(force: bool = false) -> void:
	...
	var original_scroll := world_map.get_node_or_null(GARRISON_CARD_PATH + "/MarginContainer/Content/GarrisonScroll") as ScrollContainer
	var original_list: Node = null
	if original_scroll != null:
		original_list = original_scroll.get_node_or_null("GarrisonList")
	if original_list == null:
		original_list = world_map.get_node_or_null(GARRISON_CARD_PATH + "/MarginContainer/Content/GarrisonList")
	if original_list == null:
		return
