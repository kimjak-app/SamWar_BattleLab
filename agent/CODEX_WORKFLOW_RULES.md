...
The full SamWar_BattleLab permission header and its canonical wording live in `agent/WORKFLOW_MANAGER.md`. Apply it to every task. Essential safety remains unchanged: do not push, delete, install software/packages, change OS or repo-external state, or make out-of-scope broad changes. Complete in-scope work, verification, required documentation, and an explicitly requested local commit autonomously.
...
- `PATCH`: a bounded correction or small feature change.
- `TRANSACTION`: a multi-file/system change that completes one player action end-to-end.
- `ARCHITECTURE MIGRATION`: a bounded change to an approved cross-system structure or contract.
- `DOCUMENTATION TRANSACTION`: a complete documentation-state change with its own acceptance criteria.

`TRANSACTION` can modify several systems, but must deliver one complete user action. Do not split a small feature into artificial version-sized patches; use the smallest coherent transaction.

### Legacy Detail: SIMPLE PATCH
- Typical examples:
  - constants
  - UI timing
  - UI scale
