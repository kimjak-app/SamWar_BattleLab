...
func _parity(action: String, gold: int = 10000, cooldown: int = 0, require_valid: bool = true) -> void:
	...
	seed(73021)
	var controller: RefCounted = _worldmap.call("_ensure_spy_controller")
	var direct: Dictionary = controller.call("execute", action, city_id)
	...
	_expect(actual == direct, "result parity " + label)
...
func _run() -> void:
	...
	_prepare()
	var controller: RefCounted = _worldmap.call("_ensure_spy_controller")
	var unknown_direct: Dictionary = controller.call("execute", "unknown", "missing")
	_prepare()
	var unknown_service: Dictionary = _worldmap.call("_apply_spy_action", "unknown", "missing")
	_expect(unknown_service == unknown_direct, "unknown action result parity")
