...
- `GameSession.has_valid_save()` now checks the same WorldMap save path used by the runtime, so the title-screen `이어하기` button detects real saves.
...
T04–T05 becomes `COMPLETE` after:

1. all four player starts can end multiple turns;
2. each turn advances once with no duplicate production, research, recovery, AI action, or invasion;
3. saving during enemy phase resumes and finishes the same transaction;
4. research completion presentation still works;
5. wounded soldier/general recovery still follows month boundaries;
6. player capture of the fourth city opens victory;
7. loss of the last player city opens defeat;
8. terminal save/load restores the same screen;
9. title-screen `이어하기` detects the save;
10. final Godot Output has no new errors or warnings.
