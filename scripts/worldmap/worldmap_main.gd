Warning: truncated output (original token count: 309848)
... 190813 bytes omitted ...
...
var _contextual_action_coordinator: WorldMapActionCoordinator = null
...
func open_contextual_worldmap_action(action_type: String, target_city_id: String) -> Dictionary:
	...
	if normalized_type == "trade":
		source_city_id = _find_contextual_trade_source_city_id(target_city_id)
	...
	return _contextual_action_coordinator.begin(normalized_type, target_city_id, source_city_id)


func cancel_contextual_worldmap_action() -> void:
	if _contextual_action_coordinator != null:
		_contextual_action_coordinator.cancel()
...
func _setup_contextual_worldmap_action_coordinator() -> void:
	if _contextual_action_coordinator != null:
		return
	_contextual_action_coordinator = WorldMapActionCoordinatorScript.new()
	_contextual_action_coordinator.name = "WorldMapActionCoordinator"
	add_child(_contextual_action_coordinator)
	_contextual_action_coordinator.presentation_requested.connect(
		_on_contextual_action_presentation_requested
	)
	_contextual_action_coordinator.action_resolved.connect(
		_on_contextual_action_resolved
	)


func _on_contextual_action_presentation_requested(
	action_type: String,
	action_id: String,
...
func _on_contextual_action_resolved(action_type: String, result: Dictionary) -> void:
	_refresh_city_hud_data_bindings()
	_refresh_left_world_status_panel()
	_refresh_unified_panel_content()
...
func _get_contextual_worldmap_action_type() -> String:
	return _contextual_action_coordinator.get_action_type() if _contextual_action_coordinator != null else ""


func _get_contextual_worldmap_action_target_city_id() -> String:
	return _contextual_action_coordinator.get_target_city_id() if _contextual_action_coordinator != null else ""


func _get_contextual_worldmap_action_source_city_id() -> String:
	return _contextual_action_coordinator.get_source_city_id() if _contextual_action_coordinator != null else ""
...
func _request_contextual_worldmap_action_presentation(action_type: String, action_id: String, target_city_id: String) -> void:
	if _contextual_action_coordinator == null:
		return
	var request_result := _contextual_action_coordinator.request_presentation(
		action_type,
		action_id,
		target_city_id
	)
	if not bool(request_result.get("ok", false)) \
			and str(request_result.get("reason", "")) != "already_pending":
		_contextual_action_coordinator.resolve_without_video(action_type, request_result)


func _resolve_contextual_worldmap_action_without_video(action_type: String, result: Dictionary) -> void:
	if _contextual_action_coordinator != null:
		_contextual_action_coordinator.resolve_without_video(action_type, result)


func present_contextual_worldmap_action_result(action_type: String, result: Dictionary) -> void:
...
func _connect_city_info_panel_actions() -> void:
	...
	var recruitment_callback := Callable(self, "_on_city_info_recruitment_r…212152 tokens truncated…delta": int(drift.get("commerce_delta", 0)),
...
func _on_unified_primary_tab_pressed(tab_id: String) -> void:
	...
	var contextual_action_type := _get_contextual_worldmap_action_type()
	if not contextual_action_type.is_empty():
		var contextual_tab := UNIFIED_PANEL_TAB_TRADE if contextual_action_type == "trade" else UNIFIED_PANEL_TAB_DIPLOMACY_SPY
...
func _on_unified_secondary_tab_pressed(tab_index: int) -> void:
	if _unified_primary_tab == UNIFIED_PANEL_TAB_DIPLOMACY_SPY:
		...
		var contextual_action_type := _get_contextual_worldmap_action_type()
		if contextual_action_type == "diplomacy" and _selected_diplomacy_spy_tab != DIPLOMACY_SPY_TAB_DIPLOMACY:
			cancel_contextual_worldmap_action()
		elif contextual_action_type == "spy" and _selected_diplomacy_spy_tab != DIPLOMACY_SPY_TAB_SPY:
			cancel_contextual_worldmap_action()
...
func _on_trade_control_mode_button_pressed(mode: String) -> void:
	...
	if _get_contextual_worldmap_action_type() == "trade" and mode != TRADE_CONTROL_MODE_MANUAL:
		cancel_contextual_worldmap_action()
