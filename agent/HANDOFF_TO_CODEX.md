...
- Extracted only the first small low-risk Defense / Battle pure helper batch from `scripts/worldmap_test.gd`.
- High-risk Defense / Battle functions remain locked in `scripts/worldmap_test.gd`, including BattleContext generation, pending invasion payloads, battle formula/stat calculation, deployment validation, deployment payload mutation, battle result mutation, save/load-adjacent behavior, turn orchestration, selection state mutation, and scene-node-heavy UI.
- No existing gameplay behavior, save/load schema, BattleContext schema, pending invasion schema, battle formula, diplomacy/spy formula changed.
...
- Recommended the first `v0.71-05 Domestic Tech Helper Extraction` batch from low-risk pure helper candidates only.
...
- Next session should run `v0.71-05 Domestic Tech Helper Extraction`.
...
- `scripts/worldmap_test.gd` was analyzed but not physically split.
- Function groups, extraction risks, schema-sensitive boundaries, and v0.71 step order are now documented.
- No gameplay behavior, save/load schema, BattleContext schema, pending invasion schema, battle formula, diplomacy/spy formula, enemy research, AI research, ship/siege persistent storage, scene, asset, or UI layout behavior changed.
- Next session should run `v0.71-01 MVP Main Scene Switch`.
...
- Runtime file touched: `scripts/worldmap_test.gd`.
- Agent docs updated: `agent/DOMESTIC_TECH_GAMEPLAY_EFFECT_INTEGRATION_MAP.md`, `agent/CURRENT_STATE.md`, `agent/NEXT_TASKS.md`, `agent/HANDOFF_TO_CODEX.md`, `agent/CHANGELOG.md`, `agent/SESSION_LOG.md`, `agent/WORLDMAP_RULES.md`, and `agent/DOMESTIC_TECH_MANUAL_QA.md`.
- PLAYER Economy / City effect integration now uses completed tech wrapper helpers and modifier helpers: `_has_completed_national_domestic_tech_mvp`, `_has_completed_city_domestic_tech_mvp`, `_get_player_city_domestic_economy_modifier_mvp`, `_get_national_domestic_economy_modifier_mvp`, and `_get_city_economy_tech_modifier_summary_mvp`.
- Existing actual turn income path `_apply_domestic_tech_city_economy_bonus_to_income_mvp` reads the modifier helpers and preserves the existing food/gold calculation shape.
- City detail resource tab and national warehouse summary display modifier-backed Domestic Tech economy summaries.
- ENEMY baseline helper `_get_enemy_city_economy_baseline_mvp` is side-effect-free and masked unless current intel reveals enough baseline fields.
- ENEMY still has no active research, completed tech storage, research cost, tech UI progression, or PLAYER completed-tech effect.
- Next session must run `v0.70-94 Defense / Battle Effect Integration`.
- Explicitly unchanged: save/load schema, active research payload schema, actual charge logic, gold/food deduction, food group order `rice -> barley -> seafood`, BattleContext schema, pending invasion schema, battle formula, diplomacy formula, spy formula, naval/siege production, troop/ship/siege counts, scenes, assets, and imports.
...
- Runtime file touched: `scripts/worldmap_test.gd`.
- This hotfix tightens QA safety around Domestic Tech duration and expected-cost display. It still does not implement resource payment, cost reservation, refund/cancel flow, paid-cost state, or cost-based research blocking.
- Domestic Tech inspector display is now state-specific: completed shows completion status without expected cost, researching shows remaining/total turns first, available shows `연구 소요 ... / 예상 비용 ... · 표시 전용`, and locked/blocked states show duration without cost-gating copy.
- `_get_domestic_tech_research_cost_plan_mvp()` keeps costs display-only and now also reports completion-time and paid-state persistence flags as false.
- `_get_domestic_tech_research_balance_summary_mvp()` now reports `cost_charged`, `cost_charged_on_completion`, `paid_cost_state_persisted`, `duration_fallback_applies_to_new_research`, and `active_research_duration_force_rewritten = false`.
- Active research normalization now preserves an existing positive `remaining_turns` when stored `duration_turns` is missing or malformed, instead of shrinking it to the new tier fallback. Positive explicit `duration_turns` in a tech definition is honored before duration hint and tier fallback.
- Explicitly unchanged: research start conditions, active slot limits, remaining-turn progress, completion mirror, duplicate completion guard, Safe Set effects, actual battle modifiers, diplomacy success formulas, spy success formulas, relation mutation, city_intel visibility, market/trade formulas, AI behavior, enemy research/effect, troop/ship/siege count mutation, BattleContext, pending invasion, tech ids/names/categories/branches/prerequisites, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
...
- Runtime file touched: `scripts/worldmap_test.gd`.
- This hotfix is QA/stability polish for PLAYER completed Domestic Tech diplomacy/spy display-safe preparation effects.
- `_get_domestic_tech_diplomacy_spy_bonus_mvp()` remains PLAYER completed national tech only, side-effect free, source-unique, and non-persistent.
- `_get_domestic_tech_city_spy_intel_bonus_mvp(city_id)` now has an explicit empty Safe Set guard. With the current empty city spy/intel mapping, it returns no local effect and city detail does not show a false/zero local spy section.
- Left national panel, Domestic Tech inspector, and selected PLAYER city detail continue to use helper-derived preparation/readiness copy without success-rate, relation, or city_intel wording.
- `_get_domestic_tech_diplomacy_spy_effect_summary_mvp()` reports display counters plus city spy/intel empty mapping/no-display flags and keeps `diplomacy_success_effects_applied = 0`, `spy_success_effects_applied = 0`, `relation_effects_applied = 0`, `city_intel_effects_applied = 0`, and `enemy_effects_applied = 0`.
- Explicitly unchanged: diplomacy success formulas, spy success formulas, relation mutation, city_intel visibility, enemy intel reveal policy, spy action results, spy detection formulas, AI diplomacy/spy behavior, enemy research/effect, BattleContext, pending invasion, market/trade formulas, battle/troop/naval/siege formulas, tech definitions, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
- Economy Safe Set, military/defense Safe Set, national policy Safe Set, naval/siege Display Safe Set, UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
## v0.70-72 Domestic Tech Diplomacy/Spy Display Safe Set Handoff
- Baseline: `v0.70-71-hotfix1 Naval/Siege Display Effect QA Polish` (`4a0e2983781bd82e2053a6d6d2903c4d14e2d066`).
- Runtime file touched: `scripts/worldmap_test.gd`.
- This pass is limited to PLAYER completed Domestic Tech diplomacy/spy display-safe preparation effects.
- `_get_domestic_tech_diplomacy_spy_bonus_mvp()` computes bonuses from PLAYER completed national techs only and does not save derived bonus state.
- Current national Safe Set ids are existing definitions only: `nation_envoy`, `nation_diplomacy_system`, `nation_tribute_system`, `nation_tribute_network`, `nation_world_diplomacy`, `nation_centralization`, `nation_bureaucracy`, `nation_intelligence_system`, `nation_intelligence_org`, and `nation_inspection_system`.
- `_get_domestic_tech_city_spy_intel_bonus_mvp(city_id)` exists for future current-id city spy/intel mapping, but the current mapping is empty because no city spy/intel tech definitions exist. The helper is still PLAYER-city, same-city, completed-only, side-effect free, and non-persistent.
- Left national panel and Domestic Tech inspector display diplomacy/spy readiness lines and unique source tech names. Copy uses readiness/preparation wording, not success-rate wording.
- `_get_domestic_tech_diplomacy_spy_effect_summary_mvp()` reports display counters and keeps `diplomacy_success_effects_applied = 0`, `spy_success_effects_applied = 0`, `relation_effects_applied = 0`, `city_intel_effects_applied = 0`, and `enemy_effects_applied = 0`.
- Explicitly unchanged: diplomacy success formulas, spy success formulas, relation mutation, city_intel visibility, enemy intel reveal policy, spy action results, spy detection formulas, AI diplomacy/spy behavior, enemy research/effect, BattleContext, pending invasion, market/trade formulas, battle/troop/naval/siege formulas, tech definitions, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
- Economy Safe Set, military/defense Safe Set, national policy Safe Set, naval/siege Display Safe Set, UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
## v0.70-71-hotfix1 Naval/Siege Display Effect QA Polish Handoff
- Baseline: `v0.70-71 Domestic Tech Naval/Siege Display Safe Set` (`2f3ddf3652f2527fd9c47d8f2bafaec6cd9e6771`).
- Runtime file touched: `scripts/worldmap_test.gd`.
- This hotfix is QA/stability polish for PLAYER city completed Domestic Tech naval/siege display-safe preparation effects.
- `_get_domestic_tech_city_naval_siege_bonus_mvp(city_id)` now reads a normalized completed city-tech map once per helper call and applies only completed `true` Safe Set ids for the same PLAYER city.
- Researching, incomplete, false, missing, malformed, other-city, enemy, unknown, and non-player city tech state still provide no naval/siege effect or bonus detail display.
- Source tech ids remain unique in helper output, city detail display, Domestic Tech inspector display, and QA summary checks.
- Naval/siege display copy uses the same helper result in city detail and inspector, with compact source display and no enemy/insufficient-intel exposure.
- `_get_domestic_tech_naval_siege_effect_summary_mvp()` reports completed/player/same-city guard flags plus `researching_has_naval_siege_effect = false`, `display_safe_only = true`, `source_techs_unique = true`, `ship_count_effects_applied = 0`, `siege_weapon_count_effects_applied = 0`, `battle_effects_applied = 0`, and `enemy_effects_applied = 0`.
- Explicitly unchanged: actual naval/siege battle effects, battle damage, troop attack/defense stats, troop count, ship count, siege weapon count, BattleContext, pending invasion, diplomacy, spy, market, trade, national policy expansion, AI research, enemy research/effect, enemy city tech exposure, tech definitions, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
- Economy Safe Set, military/defense Safe Set, national policy Safe Set, UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
## v0.70-71 Domestic Tech Naval/Siege Display Safe Set Handoff
- Baseline: `v0.70-70-hotfix1 National Policy Effect QA Polish` (`4bde6a04f54eafff883ea0a9044c4539b0936d17`).
- Runtime file touched: `scripts/worldmap_test.gd`.
- This pass is limited to PLAYER city completed Domestic Tech naval/siege display-safe preparation effects.
- `_get_domestic_tech_city_naval_siege_bonus_mvp(city_id)` computes bonuses from completed city techs for the same PLAYER city and does not save derived bonus state.
- Safe Set ids are limited to existing definitions: `commerce_shipyard`, `naval_training`, `naval_warship_building`, `naval_panokseon`, `naval_turtle_ship`, `naval_crane_wing_formation`, `naval_fire_ship`, `naval_cannon_mount`, `mil_siege_unit`, and `mil_siege_engine`.
- City detail and Domestic Tech inspector display selected PLAYER city naval/siege preparation bonus lines and source tech names. Enemy or insufficient-intel cities do not expose bonus detail.
- Naval/siege values are display/preparation values only. They do not create ships, siege weapons, troops, BattleContext fields, pending invasion modifiers, or battle formula modifiers.
- `_get_domestic_tech_naval_siege_effect_summary_mvp()` reports display counts plus `ship_count_effects_applied = 0`, `siege_weapon_count_effects_applied = 0`, `battle_effects_applied = 0`, and `enemy_effects_applied = 0`.
- Explicitly unchanged: actual naval/siege battle effects, battle damage, troop attack/defense stats, troop count, ship count, siege weapon count, BattleContext, pending invasion, diplomacy, spy, market, trade, national policy expansion, AI research, enemy research/effect, enemy city tech exposure, tech definitions, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
- Economy Safe Set, military/defense Safe Set, national policy Safe Set, UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
- Runtime file touched: `scripts/worldmap_test.gd`.
- This pass is the first Safe Set for PLAYER completed national Domestic Tech policy effects.
- `_get_domestic_tech_national_policy_bonus_mvp()` is side-effect free, uses completed national tech only, ignores researching/incomplete tech, keeps source tech ids unique, and does not save computed bonus state.
- Safe Set ids are limited to law reform, bureaucracy, centralization, tax reform, conscription, logistics system, population policy, foundation storage, and national monopoly.
- `tax_gold_percent` is connected once to PLAYER city gold income through the existing Domestic Tech economy path and remains clamped to `0+`.
- Admin, recruit, logistics, population, law/order, and storage policy values are display/preparation/QA effects only.
- Left national panel and Domestic Tech inspector display current national policy bonus lines and source national tech names without exposing enemy tech effects.
- `_get_domestic_tech_national_policy_effect_summary_mvp()` reports policy counters plus battle, troop stat/count, diplomacy, spy, market, and enemy effect counters fixed at 0.
- Explicitly unchanged: enemy national effects, AI research, diplomacy success, spy success, market/trade formulas, battle formulas, troop stats, troop counts, BattleContext, pending invasion, tech definitions, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
- Economy Safe Set, military/defense Safe Set, UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
- Runtime file touched: `scripts/worldmap_test.gd`.
- This pass is limited to PLAYER city completed Domestic Tech military/defense display-safe effects.
- `_get_domestic_tech_city_military_defense_bonus_mvp(city_id)` computes bonuses from completed city techs only and does not save derived bonus state.
- Safe Set ids are limited to barracks, infantry/archer/cavalry training, and wall/moat/watchtower/beacon/iron defense techs. Naval, siege, national military policy, and enemy effects remain excluded.
- City detail and Domestic Tech inspector display selected PLAYER city military/defense bonus lines and source tech names. Enemy or insufficient-intel cities do not expose bonus detail.
- Defense bonus is connected only to displayed city defense value; training/recruit capacity remains display/preparation text only.
- `_get_domestic_tech_military_defense_effect_summary_mvp()` reports `city_defense_effects_applied`, `training_display_effects_applied`, `battle_effects_applied = 0`, and `enemy_effects_applied = 0`.
- Explicitly unchanged: battle damage, troop attack/defense stats, troop count, automatic recruitment, BattleContext, pending invasion, diplomacy, spy, market, trade, national policy numeric effects, AI research, enemy research/effect, enemy city tech exposure, tech definitions, scenes, assets, icon PNGs, UI64 PNGs, and `.import` files.
- Economy Safe Set, UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
## v0.70-68-hotfix1 Domestic Tech Economy Effect QA Polish Handoff
- Baseline: `v0.70-68 Domestic Tech Numeric Effects Phase 1 - Economy Safe Set` (`1fc67e044fa02c56f8a00a7f680b3b734a88eae1`).
- Runtime file touched: `scripts/worldmap_test.gd`.
- This hotfix is QA/stability polish for the first Domestic Tech economy numeric slice.
- `_get_domestic_tech_city_economy_bonus_mvp(city_id)` remains side-effect free and computes only from completed city techs for the same PLAYER city.
- The helper now explicitly verifies city-scope definitions and agri/fish/commerce categories before applying Safe Set values.
- Researching, incomplete, false, missing, malformed, other-city, national, and enemy tech state still provide no economy bonus.
- Source tech ids are de-duplicated before UI display, domestic turn summary display, and QA summary counting.
- Food/gold numeric application uses a shared non-negative clamp helper; supply values remain display/QA-only for this phase.
- Domestic turn result text now includes a compact economy bonus line based on the same helper used by city detail and the Domestic Tech inspector; no structured computed bonus state is stored.
- QA summary reports numeric economy count, agri/fish/commerce counts, `market_effects_applied = 0`, and same-city/researching/no-persist safety flags.
- Explicitly unchanged: resource payment, combat/troop/battle/diplomacy/spy/market effects, national policy numeric effects, AI research, enemy research/effect, enemy city tech exposure, BattleContext, pending invasion, scenes, tech definitions, assets, icon PNGs, UI64 PNGs, and `.import` files.
- UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
...
## v0.70-68 Domestic Tech Numeric Effects Phase 1 - Economy Safe Set Handoff
- Baseline: `v0.70-67-hotfix1 Domestic Tech Effect Phase 1 QA Polish` (`8a9067339802bd28fecbdae810ab9ff8f7f69f91`).
- Runtime file touched: `scripts/worldmap_test.gd`.
- This pass is the first numeric Domestic Tech slice and is limited to PLAYER city economy Safe Set effects.
- `_get_domestic_tech_city_economy_bonus_mvp(city_id)` computes bonuses from completed city techs only and does not save derived bonus state.
- Safe Set ids are limited to agri/fish/commerce economy techs. National completed techs do not provide numeric bonus in this version.
- Existing player domestic income now applies completed-city-tech food and gold bonuses per city; researching/incomplete techs and other-city completions provide no effect.
- Supply bonus values are reported by helper/UI for QA visibility, but no separate supply resource state was added.
- City detail and Domestic Tech inspector display selected PLAYER city economy bonus lines and source tech names; enemy or insufficient-intel cities do not expose bonus detail.
- `_get_domestic_tech_effect_phase1_summary_mvp()` and `_get_domestic_tech_numeric_effect_phase1_summary_mvp()` report numeric economy effect counts while combat/diplomacy/spy/enemy effect counters remain 0.
- Explicitly unchanged: resource payment, combat/troop/battle/diplomacy/spy/market effects, national policy numeric effects, AI research, enemy research/effect, enemy city tech exposure, BattleContext, pending invasion, scenes, tech definitions, assets, icon PNGs, UI64 PNGs, and `.import` files.
- UI64 priority, node-click latency guard, overlay lifecycle, active/completed normalize, research start/progress/completion, and enemy/insufficient-intel safety remain locked.
