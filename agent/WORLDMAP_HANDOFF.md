...
다른 7개 메뉴는 이번 단계에서는 시각 selection/hover까지만 검증하고 실제 기능은 추가하지 않는다.
...
- Hover 라벨이 즉시 읽히면서 문양을 완전히 죽이지 않는가
- 선택 상태 라벨이 현재 메뉴를 명확히 표시하는가
- Hover/Selected 때문에 메뉴 row가 좌우로 흔들리지 않는가
