class_name WorldMapWarehouseTabsController
extends Node
...
func _install() -> void:
	if world_map == null:
		push_warning("WorldMap Warehouse Tabs: production WorldMap is missing.")
		return
	_left_panel = world_map.get_node_or_null(LEFT_PANEL_PATH) as Control
	_fallback_source_label = world_map.get_node_or_null(RESOURCE_LABEL_PATH) as Label
