class_name WorldMapTurnSummaryController
extends Node
...
func _install() -> void:
	if world_map == null:
		push_warning("WorldMap Turn Summary: ProductionWorldMap is missing.")
		return

	_popup = _ensure_popup()
	_last_market_prices = _parse_market_prices(_collect_post_turn_log_text())
	_hide_post_turn_log_nodes()
	call_deferred("_bind_turn_compass", 0)


func _bind_turn_compass(attempt: int) -> void:
	if world_map == null:
		return
	var compass := world_map.get_node_or_null("WorldMapUI/TurnEndCompass")
	if compass == null or not compass.has_signal("turn_end_requested"):
		if attempt < 12:
			call_deferred("_bind_turn_compass", attempt + 1)
		else:
			push_warning("WorldMap Turn Summary: TurnEndCompass is missing.")
		return
	var callback := Callable(self, "_on_turn_end_requested")
	if not compass.is_connected("turn_end_requested", callback):
...
func _on_turn_end_requested() -> void:
	if _watch_active or world_map == null:
		return
...
func _await_turn_completion_and_show() -> void:
	...
	while Time.get_ticks_msec() < deadline:
		...
		var turn_end_button := world_map.get_node_or_null(TURN_END_BUTTON_PATH) as Button
	...
	_watch_active = false
	set_process(false)
	push_warning("WorldMap Turn Summary: timed out waiting for completed turn state.")
...
func _ensure_popup() -> Control:
	...
	if popup == null:
		push_warning("WorldMap Turn Summary: failed to instantiate popup scene.")
		return null
...
func _hide_post_turn_log_nodes() -> void:
	if world_map == null:
		return
	var content := world_map.get_node_or_null(LEFT_CONTENT_PATH) as VBoxContainer
	if content == null:
		return
	var turn_end_button := world_map.get_node_or_null(TURN_END_BUTTON_PATH) as Button
...
func _get_turn_token() -> String:
	if world_map == null:
		return ""
	var turn_label := world_map.get_node_or_null(TURN_LABEL_PATH) as Label
	var calendar_label := world_map.get_node_or_null(CALENDAR_LABEL_PATH) as Label
