...
func _run() -> void:
	...
	_expect(_worldmap.call("_normalize_diplomacy_resource_package", raw_package) == normalized, "normalization wrapper delegates")
	...
	var wrapped_failure: Dictionary = _worldmap.call("_build_diplomacy_action_failure_result", "tribute", validation)
	_expect(direct_failure == wrapped_failure, "failure result wrapper delegates")
