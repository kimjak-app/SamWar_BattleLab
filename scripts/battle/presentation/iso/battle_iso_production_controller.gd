extends "res://scripts/battle/battle_production_controller.gd"
class_name BattleIsoProductionController

## Production isometric presentation.
## Combat authority remains on the inherited logical orthogonal grid.
## After logical state initialization, only screen projection and presentation
## are converted to the approved 3/4 isometric board.
...
const ISO_PRODUCTION_MARKER := "ISO_PRODUCTION_V1"
...
func _ready() -> void:
	# IMPORTANT: let the production battle derive its logical grid cells from
	...
	set_meta("iso_movement_experiment", ISO_PRODUCTION_MARKER)
	print("[ISO_PRODUCTION] ", ISO_PRODUCTION_MARKER, " active · ", battle_grid_controller.describe_grid())
...
func _install_iso_grid_projection() -> void:
	...
	if source_controller == null:
		push_error("[ISO_PRODUCTION] BattleGridController missing")
		return
...
func _snap_deployed_units_to_iso_grid() -> void:
	...
	for unit_state in _get_all_unit_states_in_slot_order():
		...
		if not battle_grid_controller.is_in_bounds(unit_state.grid_cell):
			push_warning("[ISO_PRODUCTION] skip out-of-bounds unit %s cell=%s" % [unit_state.display_name, unit_state.grid_cell])
			continue
		_sync_resumed_unit_markers_to_grid(unit_state)


func _sync_primary_ally_runtime_cache_to_iso_grid() -> void:
	# ally_main_01 (Yi Sun-sin in the production battle) is special in the inherited
	...
	print("[ISO_PRODUCTION] primary ally cache synced unit=", current_ally_unit_position, " portrait=", current_ally_portrait_position)
