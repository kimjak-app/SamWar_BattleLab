...
var tile_inner_color := Color(0.16, 0.62, 1.0, 0.07)
...
var inner_scale := 0.64
var outline_width := 2.0
var highlight_width := 1.0
...
func set_tile_style(fill_color: Color, outline_color: Color, highlight_color: Color) -> void:
	...
	tile_inner_color = Color(fill_color.r, fill_color.g, fill_color.b, fill_color.a * 0.42)
...
func _draw() -> void:
	...
	var inner_points := _make_octagon_points(size, inner_scale)
	var closed_outer := outer_points.duplicate()
	closed_outer.append(outer_points[0])
	var closed_inner := inner_points.duplicate()
	closed_inner.append(inner_points[0])

	draw_polygon(outer_points, PackedColorArray([tile_fill_color]))
	draw_polygon(inner_points, PackedColorArray([tile_inner_color]))
	draw_polyline(closed_outer, tile_outline_color, outline_width, true)
	draw_polyline(closed_inner, tile_highlight_color, highlight_width, true)
