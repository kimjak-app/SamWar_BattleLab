...
- Switched the active ally turn pulse to prefer the shared `UnitVisualRoot` / visual root instead of pulsing the troop token and portrait separately.
- The troop token and hero portrait now scale together as one unified object through the same root.
- Added root-base-scale capture and exact restore so repeated pulses do not accumulate scale drift.
- Kept the old token/portrait pulse path only as a fallback when a shared visual root is unavailable.
- Kept ally turn start behavior unchanged:
  - floating command panel stays hidden by default
  - pulse remains the primary active-unit signal
- Kept click-to-open panel behavior and post-move panel auto-reopen intact.
...
- Confirmed headless `Battle_Fullscreen_Test.tscn` launch exit code `0`.
- Confirmed `GDScript` warning count `0`.
- Confirmed dedicated headless unified-root verifier checks:
  - no auto-open panel at ally turn start
  - root pulse peak around `1.400x`
  - token global pulse peak around `1.403x`
  - portrait global pulse peak around `1.400x`
  - token local scale stayed around `1.010x`
  - portrait local scale stayed around `1.000x`
  - root returns to `1.000x`
  - token returns to about `1.003x`
  - portrait returns to about `1.000x`
  - panel reopens after direct move + facing completion
  - repeated-turn scale drift was not detected
- Confirmed the existing floating-panel UX verifier still passes, including floating attack/move/wait behavior, direct move-click, rollback, panel hide rules, bottom command bar survival, and accelerated `5v5` auto-battle result reachability.
...
- Updated `scripts/battle_web_import_test.gd` and agent docs only.
- Added a strong active-ally turn pulse on the current ally token instead of auto-opening the floating command panel.
- Pulse tuning:
  - `ACTIVE_ALLY_TURN_PULSE_SCALE = 1.40`
  - `ACTIVE_ALLY_TURN_PULSE_UP_DURATION = 0.18`
  - `ACTIVE_ALLY_TURN_PULSE_DOWN_DURATION = 0.24`
- Kept token base scale restoration exact after the pulse and restarted idle breathing afterward.
- Changed floating command panel visibility so ally turn start keeps it hidden by default.
- Floating command panel now opens only after the current active ally is explicitly selected.
- Kept existing floating panel opacity, high layer priority, bottom command bar, direct move-click UX, and button handler reuse intact.
- Confirmed headless project launch exit code `0`.
- Confirmed headless `Battle_Fullscreen_Test.tscn` launch exit code `0`.
- Confirmed `GDScript` warning count `0`.
- Confirmed dedicated headless pulse verifier checks:
  - no auto-open panel at ally turn start
  - pulse peak around `1.398x`
  - token returns to about `1.001x`
  - selecting the active ally opens the panel
- Confirmed the existing floating-panel UX verifier still passes, including button flow reuse, direct move-click, rollback, hide rules, bottom command bar survival, and accelerated `5v5` auto-battle result reachability.
...
- Raised the floating ally command panel draw priority so it renders above unit-facing arrows:
  - panel `z_index = 200`
  - floating button `z_index = 201`
- Kept the panel and floating buttons fully opaque.
- Kept panel/button mouse filtering in blocking mode so clicks do not pass through to battlefield elements.
- Kept battle logic, movement logic, direct move-click UX, enemy AI, formulas, toast logic, and assets unchanged.
- Confirmed headless project launch exit code `0`.
- Confirmed headless `Battle_Fullscreen_Test.tscn` launch exit code `0`.
- Confirmed `GDScript` warning count `0`.
- Confirmed a dedicated headless layer verifier reported the floating panel above all facing indicators.
- Confirmed the existing floating-panel UX verifier still passed, including bottom bar survival, floating button flow, direct move click, rollback, and accelerated `5v5` auto-battle result reachability.

## v0.67p-2 Direct Move Click UX + Floating Panel Opacity Hotfix

- Updated `Battle_Fullscreen_Test.tscn`, `scripts/battle_web_import_test.gd`, and agent docs.
- Restored direct move-click UX so a valid highlighted move cell can execute movement immediately without pressing `이동`.
- Reused the existing move execution path and preserved post-move facing selection.
- Preserved right-click rollback after direct movement.
- Added direct-move safety guards so it does not trigger during UI clicks, invalid clicks, occupied cells, enemy turn, result-finalized states, attack-select, or resolving states.
- Made the floating ally command panel visually solid:
  - fully opaque panel background
  - fully opaque button backgrounds for normal / hover / pressed / disabled states
- Hid the bottom-bar `전역 명령` label.
- Kept the bottom bar limited to:
  - `턴 종료`
  - `자동전투`
  - disabled `후퇴` placeholder
- Confirmed headless project launch exit code `0`.
- Confirmed headless `Battle_Fullscreen_Test.tscn` launch exit code `0`.
- Confirmed `GDScript` warning count `0`.
- Confirmed headless UX verifier checks for:
  - floating panel visibility after ally selection
  - fully opaque floating panel/button style alpha
  - hidden bottom-bar label
  - bottom auto / end-turn / retreat-placeholder survival
  - direct valid move click
  - facing-select continuation after direct move
  - right-click rollback after direct move
  - invalid click rejection
  - UI click direct-move rejection
...
- No code, scene, or asset changes in this step.
- Locked today’s wrap-up baseline as `v0.67k-4 Enemy AI Surround Pressure QA Stable with Known Issue`.
- Recorded stable areas:
  - `5v5` MVP structure
  - Hero Identity Registry
  - reinforcement arrival toast
  - victory / defeat result toast
  - auto battle completion path
  - improved enemy surround engagement behavior
- Recorded Known Issue:
  - some enemy actors still idle in multi-target battles even when alternate engagement is possible
  - likely missing target / destination reservation behavior
- Recorded next-session follow-up:
...
- Updated `scripts/battle_web_import_test.gd` and agent docs only.
- Kept `Battle_Fullscreen_Test.tscn` unchanged.
- Reworked enemy destination selection so surround / engagement candidates are always considered, not only in low-ally pressure mode.
- Added enemy engagement helpers:
  - `_get_enemy_engagement_candidate_cells()`
  - `_get_enemy_engagement_step_plan_for_actor()`
  - `_find_enemy_path_to_destination_for_actor()`
- Enemy AI now advances toward the final engagement ring even when it cannot reach the final surround cell in one turn.
- Added clearer endgame logs:
...
- Documentation-only stable-mark step.
- No code, scene, or asset changes in this step.
- Locked today’s stable baseline as `v0.67k-4 Enemy AI Surround Pressure QA Stable`.
- Recorded the current validated behavior:
  - `5v5` MVP structure remains stable
  - enemy AI presses the last `1~2` surviving allies instead of unnecessary idle
  - front-blocked approach falls back to side / rear surround cells
  - wait only occurs when no useful path exists
  - victory / defeat toast remains stable
  - reinforcement arrival toast remains stable
  - hero identity registry remains stable
  - full auto battle can reach battle-end result flow
- Recorded headless validation:
  - project launch exit code `0`
  - `Battle_Fullscreen_Test.tscn` launch exit code `0`
  - `GDScript` warning 없음
  - endgame approach / surround logs confirmed
...
- Updated `scripts/battle_web_import_test.gd` and agent docs only.
- Kept `Battle_Fullscreen_Test.tscn` unchanged.
- Improved enemy destination selection priority:
  - attack from current cell
  - move to an attackable cell
  - move to a surround cell near the target
  - otherwise reduce distance
- Added surround-pressure mode when deployed alive allies are `<= 2` and enemies outnumber them.
