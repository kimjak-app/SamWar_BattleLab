...
## Production current-actor / bottom-HUD binding controller.
...
const CURRENT_ACTOR_INFO_HUD_SCENE := preload("res://scenes/battle/ui/CurrentActorInfoHud.tscn")
...
const DEFAULT_BATTLEFIELD_TERRAIN_NAME := "평지"
const DEFAULT_BATTLEFIELD_TERRAIN_DEFENSE := "방어 0%"
const DEFAULT_BATTLEFIELD_TERRAIN_MOVE := "이동 0%"
...
func _ensure_current_actor_info_hud_preview() -> void:
	...
	if production_root.get_node_or_null("CurrentActorInfoHud") == null:
		var preview_hud := CURRENT_ACTOR_INFO_HUD_SCENE.instantiate() as Control
		if preview_hud != null:
...
func _sync_terrain(controller: Node, hud: Control) -> void:
	var context: Dictionary = {}
	if controller != null:
		var value: Variant = controller.get("worldmap_battle_context")
		if value is Dictionary:
			context = value
	var terrain_name := str(context.get("terrain_name", DEFAULT_BATTLEFIELD_TERRAIN_NAME)).strip_edges()
	var terrain_defense := str(context.get("terrain_defense_text", DEFAULT_BATTLEFIELD_TERRAIN_DEFENSE)).strip_edges()
	var terrain_move := str(context.get("terrain_move_text", DEFAULT_BATTLEFIELD_TERRAIN_MOVE)).strip_edges()
	if terrain_name.is_empty():
		terrain_name = DEFAULT_BATTLEFIELD_TERRAIN_NAME
	if terrain_defense.is_empty():
		terrain_defense = DEFAULT_BATTLEFIELD_TERRAIN_DEFENSE
	if terrain_move.is_empty():
		terrain_move = DEFAULT_BATTLEFIELD_TERRAIN_MOVE
	_set_local_text(hud, "TerrainArea/TerrainNameLabel", terrain_name)
	_set_local_text(hud, "TerrainArea/TerrainImageSlot/PlaceholderLabel", terrain_name)
	_set_local_text(hud, "TerrainArea/TerrainEffectLabel", "%s\n%s" % [terrain_defense, terrain_move])
