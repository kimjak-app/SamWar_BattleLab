...
- 김작 F6 visual QA should confirm `v0.68a-fix4` status badge blocks snap to the facing-arrow edge with `0-4px` visual gap, up/down-facing badges do not sit inside the body center, confusion renders as `◎N`, shake remains `⚠N`, and multi-icon badges stay horizontally aligned.
