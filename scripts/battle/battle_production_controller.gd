...
class_name BattleProductionController

## Generic production battle behavior promoted from the latest battle QA path.
## Scenario identity/roster/portrait overrides remain in test fixtures.
...
const PRODUCTION_OVERLAY_RESYNC_INTERVAL := 0.05
const PRODUCTION_ACTIVE_ACTOR_CAMERA_Y_OFFSET := 120.0
const PRODUCTION_POSITIVE_STATUS_IDS := {
...
var _production_overlay_resync_accumulator := 0.0
var _production_auto_reposition_skip_queued := false

@onready var production_cutin_input_blocker: Control = get_node_or_null("HeroCutinOverlay/HeroCutinInputBlocker") as Control


func _process(delta: float) -> void:
	super._process(delta)
	_sync_production_cutin_input_blocker()
	_production_overlay_resync_accumulator += delta
	if _production_overlay_resync_accumulator >= PRODUCTION_OVERLAY_RESYNC_INTERVAL:
		_production_overlay_resync_accumulator = 0.0
	...
	if current_phase == PHASE_POST_SKILL_REPOSITION_SELECT \
			...
			and not _production_auto_reposition_skip_queued:
		_production_auto_reposition_skip_queued = true
		call_deferred("_finish_production_auto_reposition_skip")


func _sync_production_cutin_input_blocker() -> void:
	if production_cutin_input_blocker == null or hero_cutin_overlay == null:
		return
	production_cutin_input_blocker.mouse_filter = Control.MOUSE_FILTER_STOP if hero_cutin_overlay.visible else Control.MOUSE_FILTER_IGNORE
...
func _finish_production_auto_reposition_skip() -> void:
	_production_auto_reposition_skip_queued = false
...
func _return_to_ally_turn() -> void:
	...
	if current_phase == PHASE_ALLY_TURN \
		...
		call_deferred("_apply_production_active_actor_safe_camera_focus", active_unit_state)


func _apply_production_active_actor_safe_camera_focus(expected_unit_state: BattleUnitState) -> void:
	...
	focus_position += Vector2(0.0, PRODUCTION_ACTIVE_ACTOR_CAMERA_Y_OFFSET)
...
func _get_unit_status_display_entries(unit_state: BattleUnitState) -> Array[Dictionary]:
	...
	for inherited_entry in inherited_entries:
		...
		if PRODUCTION_POSITIVE_STATUS_IDS.has(status_id):
