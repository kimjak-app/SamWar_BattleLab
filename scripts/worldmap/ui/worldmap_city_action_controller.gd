class_name WorldMapCityActionController
extends Node
...
func _install() -> void:
	...
	if city_layer == null:
		push_warning("WorldMap City Action: CityLayer is missing.")
		return
...
func _build_action_menu() -> void:
	...
	_canvas_layer.name = "CityActionCanvas"
	_canvas_layer.layer = 35
	add_child(_canvas_layer)

	_panel = PanelContainer.new()
	_panel.name = "CityActionPanel"
...
func _on_city_selected(marker: Node) -> void:
	if world_map != null and world_map.has_method("cancel_contextual_worldmap_action"):
		world_map.call("cancel_contextual_worldmap_action")
...
func _sync_battle_button_state() -> void:
	if _battle_button == null or world_map == null:
		return
	var legacy_attack_button := world_map.get_node_or_null(LEGACY_ATTACK_BUTTON_PATH) as Button
...
func _on_contextual_action_pressed(action_type: String) -> void:
	if world_map == null or _selected_marker == null:
		return
	var target_city_id := str(_selected_marker.get("city_id"))
	if target_city_id.is_empty():
		return
	if ["diplomacy", "spy", "trade"].has(action_type):
		var result: Dictionary = world_map.call("open_contextual_worldmap_action", action_type, target_city_id)
		if not bool(result.get("ok", false)):
			world_map.call("_resolve_contextual_worldmap_action_without_video", action_type, result)
	_hide_menu()
