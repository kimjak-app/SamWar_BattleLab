class_name WorldMapReadabilityController
extends Node
...
func _install() -> void:
	if world_map == null:
		push_warning("WorldMap Readability: production WorldMap is missing.")
		return
...
func _compact_calendar_text() -> void:
	var calendar := world_map.get_node_or_null(CALENDAR_LABEL_PATH) as Label
	if calendar == null:
		return
...
func _refine_chancellor_card() -> void:
	var card := world_map.get_node_or_null(CHANCELLOR_CARD_PATH)
	if card == null:
		return
	var assignment := world_map.get_node_or_null(CHANCELLOR_ASSIGNMENT_PATH) as OptionButton
...
func _on_city_selected(_marker: Node) -> void:
	call_deferred("_refresh_selected_city_readability")


func _refresh_selected_city_readability() -> void:
	_refine_governor_card()
	_request_initial_refit()
