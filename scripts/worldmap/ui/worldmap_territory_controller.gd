class_name WorldMapTerritoryController
extends Node
...
func _ready() -> void:
	call_deferred("_initialize_territory_layer")
...
func _initialize_territory_layer() -> void:
	if LAND_SEA_MASK.get_width() != EXPECTED_MASK_SIZE.x or LAND_SEA_MASK.get_height() != EXPECTED_MASK_SIZE.y:
		push_error(
			"WorldMap Territory: expected %dx%d land/sea mask, got %dx%d."
			% [
	...
	_initialized = true
	if world_map == null:
		push_error("WorldMap Territory: production WorldMap root is missing.")
		return

	var world_root := world_map.get_node_or_null("WorldMapRoot")
	if world_root == null:
		push_error("WorldMap Territory: WorldMapRoot is missing.")
		return
	...
	if route_layer == null or city_layer == null:
		push_error("WorldMap Territory: RouteLayer or CityLayer is missing.")
		return
...
func refresh_territory_data() -> void:
	...
	var city_layer := world_root.get_node_or_null("CityLayer")
	if city_layer == null:
		push_warning("WorldMap Territory: CityLayer is missing during refresh.")
		return
	...
	for child in city_layer.get_children():
		if city_count >= MAX_CITIES:
			push_warning("WorldMap Territory: MAX_CITIES=%d reached." % MAX_CITIES)
			break
		...
		if not owner_colors.has(faction_id):
			push_warning(
				"WorldMap Territory: owner color missing for city %s faction %s."
