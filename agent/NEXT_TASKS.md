...
- Completed direction:
  ...
  2. Kept diplomacy/spy formula, relation mutation, spy result mutation, spy detection/relation penalty mutation, city loyalty/security/public support mutation, save/load, turn orchestration, selection state mutation, and scene-node-heavy UI logic in `scripts/worldmap_test.gd`.
  3. Preserved the v0.71-07a opportunistic extraction ban for Debug / QA, Enemy Baseline, and Mixed / Unsafe.
  4. Preserved existing scene paths, script paths, gameplay formulas, schemas, and v0.71 refactor safety boundaries.
- Next task:
  1. `v0.71-09 Naval / Siege Helper Extraction`.
...
- Completed direction:
  ...
  2. Kept active research mutation, actual charge, completion queue, save/load, BattleContext, pending invasion, turn orchestration, and scene-node-heavy UI logic in `scripts/worldmap_test.gd`.
  3. Preserved existing scene paths, script paths, gameplay formulas, schemas, and v0.71 refactor safety boundaries.
- Next task:
  1. `v0.71-06 Economy / City Helper Extraction`.
...
- Completed direction:
  1. Verified Research Flow, Economy / City, Defense / Battle, Diplomacy / Spy, Naval / Siege, Enemy Baseline / No Enemy Research, Preservation, and Godot Output QA gates.
  2. Confirmed v0.70-93 through v0.70-96 integrations remain connected to their existing helpers and hooks.
  3. Confirmed PLAYER completed tech does not leak to enemy baselines.
  4. Confirmed city completed tech remains same-city only.
  5. Confirmed no blocker requiring `v0.70-97-hotfix1` was found.
...
- Route lock:
  ...
  4. Do not add new gameplay systems before the Complete Lock.
  5. The next task must complete the lock, not add more effect integrations.
- Next task:
  1. `v0.70-98 Domestic Tech Complete Lock`.
...
- Completed direction:
  1. Recorded Kimjak's v0.70-82 actual charge manual QA result as PASS.
  2. Added the PASS result block to `agent/DOMESTIC_TECH_MANUAL_QA.md`.
  3. Preserved the v0.70-81 actual charge MVP and v0.70-82 checklist scope without code behavior changes.
- PASS coverage:
  1. National sufficient/insufficient gold, no per-turn charge, and no completion charge.
  2. City sufficient gold/food, insufficient gold, insufficient food, food deduction order, and other city unchanged.
  3. Existing active no retroactive charge, active payload schema unchanged, enemy/unknown no research/effect, and Godot Output clean.
- Next candidates:
...
- Completed direction:
  1. Reconfirmed the grace turn boundary as 1-based turns 1-10 blocked and turn 11+ restored to existing invasion/pressure logic.
  2. Kept guards scoped to new pending invasion creation, enemy pressure plan creation, and strategic pressure follow-up creation.
  3. Expanded `_get_manual_qa_grace_summary_mvp()` with QA flags for pending invasion creation, enemy pressure, strategic follow-up, existing pending invasion deletion, income, UI refresh, BattleContext, pending invasion schema, and enemy AI global-disable state.
  4. Documented that existing pending invasions are not deleted or schema-rewritten by the grace.
- Preserved scope:
  1. No BattleContext structure change, pending invasion schema change, pending invasion mass cleanup, enemy AI global disable, Domestic Tech research/effect/cost change, cost charge/gating, Safe Set regression, tech definition change, asset/icon/UI64/import change, or large refactor was made.
  2. Turn progress, Domestic Tech research progress/completion, income, and UI refresh remain active during grace.
- Next candidates:
  1. `v0.70-77 Domestic Tech Manual QA Scenario Pack`.
  2. `v0.70-77 Domestic Tech Actual Manual QA Pass`.
- Manual F6 QA required:
  1. Check turn 1-10 no new invasion/pending invasion/pressure/follow-up creation, turn 11 logic return, research progress/completion during grace, income during grace, UI64/click/overlay, and clean Godot Output.
...
- Completed direction:
  1. Strengthened naval/siege display helper around normalized PLAYER city completed-tech state and completed `true` Safe Set ids only.
  2. Kept same-city-only behavior, enemy/no-display guard, and non-persistent computed naval/siege bonus state.
  3. Kept naval/siege values display/preparation-only and added QA summary flags for researching no-effect, source uniqueness, and display-safe-only.
  4. Kept city detail and Domestic Tech inspector display aligned with the same helper output.
- Preserved scope:
...
- Next candidate:
  1. `v0.70-72 Domestic Tech Diplomacy/Spy Display Safe Set`.
- Manual F6 QA required:
...
- Completed direction:
  1. Added `_get_domestic_tech_city_military_defense_bonus_mvp(city_id)` for completed city Domestic Tech only.
  2. Kept PLAYER-city-only, same-city-only, completed-only behavior and no persisted computed bonus state.
  3. Added Safe Set mapping for recruitment capacity display, infantry/archer/cavalry training display, and city defense display.
  4. Added selected PLAYER city military/defense bonus/source display to city detail and Domestic Tech inspector.
  5. Extended QA summary with city defense count, training display count, `battle_effects_applied = 0`, and enemy/no-persist safety flags.
- Preserved scope:
  1. No battle formula, troop stat, troop count, BattleContext, pending invasion, diplomacy, spy, market, trade, AI, enemy effect/research, naval numeric, siege numeric, tech definition, asset, icon, UI64, or `.import` change was made.
  2. Economy Safe Set, UI64 priority, click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, and research start/progress/completion remain locked.
- Next candidates:
  1. `v0.70-69-hotfix1 Military/Defense Effect QA Polish`.
  2. `v0.70-70 Domestic Tech National Policy Effects Safe Set`.
- Manual F6 QA required:
  1. Check city military/defense bonus, same-city only, enemy no-effect, no battle mutation, no troop count mutation, UI bonus display, research flow, economy Safe Set, icon visibility, click latency, overlay lifecycle, and clean Godot Output.
...
- Completed direction:
  1. Added completed-city-tech economy bonus helper for PLAYER cities only.
  2. Added agri/fish/commerce Safe Set mapping for food, gold, and display-level supply bonus values.
  3. Hooked food/gold Safe Set bonuses into the existing player domestic income path without storing bonus state.
  4. Added city detail and Domestic Tech inspector bonus/source display for selected PLAYER cities.
  5. Extended QA summary output with numeric economy count and combat/diplomacy/spy/enemy effect counters fixed at 0.
- Preserved scope:
  1. Researching/incomplete techs do not apply effects; other-city completed techs do not transfer.
  2. Enemy cities receive no economy effect and no bonus display.
  3. No combat/troop/battle/diplomacy/spy/market effect, national policy numeric effect, AI research, enemy research/effect, BattleContext/pending invasion change, asset/import change, or tech definition change was made.
  4. UI64 icon priority, click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, and research start/progress/completion remain locked.
- Next candidates:
  1. `v0.70-68-hotfix1 Economy Effect QA Polish`.
  2. `v0.70-69 Domestic Tech Military/Defense Effects Safe Set`.
- Manual F6 QA required:
  1. Check city economy bonus, turn income change, same-city only, enemy no-effect, UI bonus display, research flow, icon visibility, click latency, overlay lifecycle, and clean Godot Output.
...
- Completed direction:
  1. Kept completed national tech as the required-national source of truth for city tech unlock conditions.
  2. Kept completed city tech prerequisites scoped to the same city only.
  3. Added explicit required-national completion helper coverage so researching/active tech is not unlock credit.
  4. Improved inspector unlock/enhance relation display around completed state: applied, unlocked, condition met, or research needed.
  5. Reframed `effect_stub` as description plus Phase 1 application-readiness copy, with no formula connection.
- Preserved scope:
  1. No numeric effects, resource payment, AI research, enemy research/effect, BattleContext/pending invasion change, or gameplay formula change was made.
  2. UI64 icon priority, click latency behavior, overlay lifecycle, PLAYER-only national/city scope, enemy/insufficient-intel hiding, and research start/progress/completion remain locked.
- Next candidate:
  1. `v0.70-67-hotfix1 Domestic Tech Effect Phase 1 QA Polish`.
  2. Alternative: `v0.70-68 Domestic Tech Numeric Effects Phase 1 - Economy Safe Set`.
- Manual F6 QA required:
  1. Check national-to-city unlock, same-city city prerequisite only, relation display, effect display, no resource/troop/battle/diplomacy/spy/market mutation, enemy-city safety, icon visibility, click latency, overlay lifecycle, and clean Godot Output.
