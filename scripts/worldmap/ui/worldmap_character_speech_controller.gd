class_name WorldMapCharacterSpeechController
extends Node
...
func _install() -> void:
	...
	_install_description_visibility_guards()
	_connect_events()
...
func _hide_description_labels() -> void:
	for path in [CHANCELLOR_DESCRIPTION_PATH, GOVERNOR_DESCRIPTION_PATH]:
		var item := world_map.get_node_or_null(path) as CanvasItem
		if item != null:
			item.visible = false


func _install_description_visibility_guards() -> void:
	for path in [CHANCELLOR_DESCRIPTION_PATH, GOVERNOR_DESCRIPTION_PATH]:
		var item := world_map.get_node_or_null(path) as CanvasItem
		if item == null:
			continue
		var callback := Callable(self, "_on_description_visibility_changed").bind(item)
		if not item.visibility_changed.is_connected(callback):
			item.visibility_changed.connect(callback)


func _on_description_visibility_changed(item: CanvasItem) -> void:
	if item != null and is_instance_valid(item) and item.visible:
		item.visible = false


func _connect_events() -> void:
	var chancellor_assign := world_map.get_node_or_null(CHANCELLOR_ASSIGN_PATH) as OptionButton
	var chancellor_policy := world_map.get_node_or_null(CHANCELLOR_POLICY_PATH) as OptionButton
	var governor_assign := world_map.get_node_or_null(GOVERNOR_ASSIGN_PATH) as OptionButton
	var governor_policy := world_map.get_node_or_null(GOVERNOR_POLICY_PATH) as OptionButton
...
func _capture_initial_state() -> void:
	_last_chancellor_id = _selected_metadata(world_map.get_node_or_null(CHANCELLOR_ASSIGN_PATH) as OptionButton)
	_last_governor_id = _selected_metadata(world_map.get_node_or_null(GOVERNOR_ASSIGN_PATH) as OptionButton)
	_last_chancellor_policy = _selected_metadata(world_map.get_node_or_null(CHANCELLOR_POLICY_PATH) as OptionButton)
	_last_governor_policy = _selected_metadata(world_map.get_node_or_null(GOVERNOR_POLICY_PATH) as OptionButton)
...
func _show_chancellor_assignment_if_changed() -> void:
	var option := world_map.get_node_or_null(CHANCELLOR_ASSIGN_PATH) as OptionButton
...
func _show_governor_assignment_if_changed() -> void:
	var option := world_map.get_node_or_null(GOVERNOR_ASSIGN_PATH) as OptionButton
	var current := _selected_metadata(option)
...
func _show_chancellor_policy_if_changed() -> void:
	var option := world_map.get_node_or_null(CHANCELLOR_POLICY_PATH) as OptionButton
	var current := _selected_metadata(option)
	if current == _last_chancellor_policy:
		return
	_last_chancellor_policy = current
	var chancellor_id := _selected_metadata(world_map.get_node_or_null(CHANCELLOR_ASSIGN_PATH) as OptionButton)
...
func _show_governor_policy_if_changed() -> void:
	var option := world_map.get_node_or_null(GOVERNOR_POLICY_PATH) as OptionButton
	var current := _selected_metadata(option)
	if current == _last_governor_policy:
		return
	_last_governor_policy = current
	var governor_id := _selected_metadata(world_map.get_node_or_null(GOVERNOR_ASSIGN_PATH) as OptionButton)
...
func _show_character(role_text: String, character_name: String, portrait_path: String, fallback_path: String, speech: String) -> void:
	if _popup == null:
		return
	var portrait_node := world_map.get_node_or_null(portrait_path) as TextureRect
	var fallback_node := world_map.get_node_or_null(fallback_path) as Label
...
func _text_at(path: String, fallback: String) -> String:
	var label := world_map.get_node_or_null(path) as Label
	if label == null:
		return fallback
